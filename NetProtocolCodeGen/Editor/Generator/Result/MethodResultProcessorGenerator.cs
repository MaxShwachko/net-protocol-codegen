...
namespace NetProtocolCodeGen.Editor.Generator.Result
{
    public class MethodResultProcessorGenerator
    {
        private const string ClassName = "MethodResultProcessor";
            ...
            List<MethodScheme> methodSchemes)
        {
            ...
            var cu = SyntaxFactory.CompilationUnit()
                .AddUsings(UsingDirectivesHolder.Instance.UsingDirectives.MethodResultProcessorGenerator);
        ...
        private ClassDeclarationSyntax GenerateTemplate(List<MethodScheme> methodSchemes)
        {
                    ...
                    SyntaxFactory.BaseList(
                        SyntaxFactory.SingletonSeparatedList<BaseTypeSyntax>(
                            SyntaxFactory.SimpleBaseType(
                                SyntaxFactory.IdentifierName("IMethodResultProcessor")))));
            ...
            var addResultListener = CreateAddResultListener(methodSchemes);
            classDeclaration = classDeclaration.AddMembers(addResultListener);
            
            var removeResultListener = CreateRemoveResultListener(methodSchemes);
            classDeclaration = classDeclaration.AddMembers(removeResultListener);
        ...
        private MethodDeclarationSyntax CreateHandle(List<MethodScheme> methodSchemes)
        {
            ...
            var parameterList = SyntaxFactory.ParameterList(
                SyntaxFactory.SingletonSeparatedList(
                    SyntaxFactory.Parameter(
                        ...
                        .WithType(
                            SyntaxFactory.IdentifierName("IMethodResult"))));
            ...
            foreach (var methodScheme in methodSchemes)
            {
                ...
                var result = GetResultName(methodScheme);

                var forEachBlock = SyntaxFactory.Block();
                forEachBlock = forEachBlock.AddStatements(SyntaxFactory.ParseStatement("listener.Handle((" + result + ")result);"));
                ...
                var ifStatement = SyntaxFactory.IfStatement(
                    SyntaxFactory.BinaryExpression(
                        ...
                        SyntaxFactory.IdentifierName("result.Id"),
                        SyntaxFactory.IdentifierName($"{nameSpace}.Id && result.AgentId == {nameSpace}.AgentId")),
                    ifBlock);
        ...
        private MethodDeclarationSyntax CreateAddResultListener(List<MethodScheme> methodSchemes)
        {
            var method = SyntaxFactory.MethodDeclaration(
                    SyntaxFactory.PredefinedType(SyntaxFactory.Token(SyntaxKind.VoidKeyword)),
                    SyntaxFactory.Identifier("AddResultListener"))
            ...
            foreach (var methodScheme in methodSchemes)
            {
                ...
                var ifStatement = SyntaxFactory.IfStatement(
                    SyntaxFactory.BinaryExpression(
                        ...
                        SyntaxFactory.IdentifierName("typeof(" + result + ")")),
                    block);
        ...
        private MethodDeclarationSyntax CreateRemoveResultListener(List<MethodScheme> methodSchemes)
        {
            var method = SyntaxFactory.MethodDeclaration(
                    SyntaxFactory.PredefinedType(SyntaxFactory.Token(SyntaxKind.VoidKeyword)),
                    SyntaxFactory.Identifier("RemoveResultListener"))
                .WithTypeParameterList(
                    SyntaxFactory.TypeParameterList(
                        SyntaxFactory.SingletonSeparatedList(
                            SyntaxFactory.TypeParameter(
        ...
        private string GetTypeName(MethodScheme methodScheme)
        {
            var mn= methodScheme.method.ToMethodName();
            var an = methodScheme.agent.FirstCharToUpper();
            return an + "." + an + "." + mn + ".IResultListener";
        }
        ...
        private string GetResultName(MethodScheme methodScheme)
        {
            var mn= methodScheme.method.ToMethodName();
            var an = methodScheme.agent.FirstCharToUpper();
            return an + "." + an + "." + mn + ".Result";
        }
    }
